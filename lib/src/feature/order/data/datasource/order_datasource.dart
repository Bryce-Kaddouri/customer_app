...
class OrderDataSource {
  ...
  Future<Either<DatabaseFailure, List<OrderModel>>> getOrdersByCustomerId(
    ...
    try {
      ...
      int customerId = user.userMetadata!['id'] as int;

      List<Map<String, dynamic>> responseFuture = await _client
          .from('all_orders_view')
          .select()
          .gte('order_date', DateTime.now().toIso8601String())
          .eq('customer ->> customer_id', customerId)
          .order('order_time', ascending: true);
      print('responseFuture');
      print(responseFuture);

      List<Map<String, dynamic>> responseOld = await _client
          .from('order_history')
          .select()
          .lt('order_date', DateTime.now().toIso8601String())
          .eq('customer ->> customer_id', customerId)
          .order('order_time', ascending: true);
      print('responseOld');
      print(responseOld);
      ...
      if (responseOld.isNotEmpty) {
        orderList
            .addAll(responseOld.map((e) => OrderModel.fromJson(e)).toList());
      }
...
*/
  ...
  Future<Either<DatabaseFailure, OrderModel>> getOrderById(
    ...
    try {
      ...
      if (date.isBefore(DateTime.now().copyWith(
